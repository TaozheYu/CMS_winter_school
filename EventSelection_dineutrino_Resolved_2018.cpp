...
void EventSelection_dineutrino_Resolved_2018(/*const char * Input = ""*/){
  ...
  readerTIncl.load(calib, BTagEntry::FLAV_B, "incl");
  readerTComb.load(calib, BTagEntry::FLAV_B, "comb");
  readerMIncl.load(calib, BTagEntry::FLAV_B, "incl");
  readerMComb.load(calib, BTagEntry::FLAV_B, "comb");
  readerLIncl.load(calib, BTagEntry::FLAV_B, "incl");
  readerLComb.load(calib, BTagEntry::FLAV_B, "comb");

  for(SysJes=0; SysJes<1; SysJes++){
    for(SysJer=0; SysJer<1; SysJer++){  
  ...
  */
  ...
  for(unsigned int Nfiles=0; Nfiles<fileName.size(); Nfiles++){
    string NewFileprov;
    //if ((SysJes==0)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/Selection_SFBtagError_veto/"+fileName[Nfiles];
    if ((SysJes==0)&&(SysJer==0)) NewFileprov = "./"+fileName[Nfiles];
    //if ((SysJes==1)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/Selection_SFBtagError_veto/JESup/"+fileName[Nfiles];
    //if ((SysJes==2)&&(SysJer==0)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/Selection_SFBtagError_veto/JESdo/"+fileName[Nfiles];
    //if ((SysJes==0)&&(SysJer==1)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/Selection_SFBtagError_veto/JERup/"+fileName[Nfiles];
    //if ((SysJes==0)&&(SysJer==2)) NewFileprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/Selection_SFBtagError_veto/JERdo/"+fileName[Nfiles];
    if ((SysJes!=0)&&(SysJer!=0)) continue;
    ...
    string FILEprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/data_and_sample_v3/"+fileName[Nfiles];
    ...
    //cout<<nentries<<endl;
    float nentries_ = std::min(nentries/10, 60000);
    //cout<<nentries_<<endl;
	...
	if(!data){
	  ...
	  GenWeight(fileName[Nfiles], GenZPt, GenWPt);
          HTSF(fileName[Nfiles],HT,Met_pt,w_ZToNuNu,w_ZToNuNuUp,w_ZToNuNuDown,w_ZToNuNu_BtagUp,w_ZToNuNu_BtagDown,w_WToLNu,w_WToLNuUp,w_WToLNuDown,w_WToLNu_BtagUp,w_WToLNu_BtagDown,w_ttbar,w_ttbarUp,w_ttbarDown,w_ttbar_BtagUp,w_ttbar_BtagDown);
...
void SelectJets(int jetType, vector<TLorentzVector> & SelectedJets, vector<float> & SelectedJetsCSV, vector<TLorentzVector> SelectedElectrons, vector<TLorentzVector> SelectedMuons, int SysJes, int SysJer, bool data, bool &deltaPhiJetMet, bool &HEMveto, float &w_HEM){
  ...
  for (UInt_t j = 0; j < Jet_pt_->size(); ++j){
    ...
    if(fabs(Jet_eta_->at(j))<2.6){
      ...
      if(!(Jet_neutralEmEnergyFraction_->at(j)<0.90))                       continue;
      if(!(Jet_neutralHadEnergyFraction_->at(j)>0.2))                      continue;
...
void SelectCA8Jets(int CA8jetType,vector<TLorentzVector> & SelectedCA8Jets,vector<TLorentzVector> SelectedElectrons,vector<TLorentzVector> SelectedMuons, vector<int> & CA8Indices,int SysJes, int SysJer, bool data, bool &deltaPhiJetMet){
  ...
  for (UInt_t j = 0; j < BoostedJet_pt_->size(); ++j){
    ...
    if(CA8jetType==0){
      ...
      if(!(BoostedJet_puppi_tau2_->at(j)/BoostedJet_puppi_tau1_->at(j)+0.082*log(BoostedJet_puppi_softdrop_mass_->at(j)*BoostedJet_puppi_softdrop_mass_->at(j)/(jetpt))<0.43)) continue;
      ...
      if(!(BoostedJet_puppi_tau3_->at(j)/BoostedJet_puppi_tau2_->at(j)<0.65)) continue;
    }
...
void BTagSF(int selection, float JetPt, float JetEta, int flav, float &SF, float &SFerr){
  ...
  if(selection==0 || selection==1){
    if(flav==5 || flav==4){
      ...
      jet_scalefactor    = readerMComb.eval_auto_bounds("central",btagentry_flav, JetEta, JetPt);
      jet_scalefactor_up = readerMComb.eval_auto_bounds("up",btagentry_flav, JetEta, JetPt);
      jet_scalefactor_do = readerMComb.eval_auto_bounds("down",btagentry_flav, JetEta, JetPt);
    } else {
      ...
      jet_scalefactor    = readerMIncl.eval_auto_bounds("central",btagentry_flav, JetEta, JetPt);
      jet_scalefactor_up = readerMIncl.eval_auto_bounds("up",btagentry_flav, JetEta, JetPt);
      jet_scalefactor_do = readerMIncl.eval_auto_bounds("down",btagentry_flav, JetEta, JetPt);
    }
  } else if(selection==2){
    if(flav==5 || flav==4){
...
void get_weight_btag(int selection, float &w_Btag, float &w_BtagUp, float &w_BtagDown, float &w_Btag1Up, float &w_Btag1Down, float &w_Btag2Up, float &w_Btag2Down, float &w_BtagLoose, float &w_BtagLooseUp, float &w_BtagLooseDown, string fileName){
  string FILEprov = "/publicfs/cms/user/yutz/Tprime/2018_dineutrino/BtagEfficiency_v3/"+fileName;
  const char *FILE = FILEprov.c_str();
...
void HTSF(string fileName, float HT, float Met_pt, float &w_SF1, float &w_SF1Up, float &w_SF1Down, float &w_SF1_BtagUp, float &w_SF1_BtagDown, float &w_SF2, float &w_SF2Up, float &w_SF2Down, float &w_SF2_BtagUp, float &w_SF2_BtagDown, float &w_SF3, float &w_SF3Up, float &w_SF3Down, float &w_SF3_BtagUp, float &w_SF3_BtagDown){
  if(fileName.find("ZToNuNu")!=string::npos ){
        ...
        if (htBin==0 || metBin==0){
           w_SF1=1; w_SF1Up=1; w_SF1Down=1; w_SF1_BtagUp=1; w_SF1_BtagDown=1;
        }  else {
           ...
           //w_SF1_BtagUp =  histoSF1_BtagUp->GetBinContent(htBin,metBin);
           //w_SF1_BtagDown =  histoSF1_BtagDown->GetBinContent(htBin,metBin);
           w_SF1_BtagUp = histoSF1->GetBinContent(htBin,metBin)+fabs(histoSF1_BtagUp->GetBinContent(htBin,metBin)-histoSF1->GetBinContent(htBin,metBin));
           w_SF1_BtagDown = TMath::Max(0.,histoSF1->GetBinContent(htBin,metBin)-fabs(histoSF1->GetBinContent(htBin,metBin)-histoSF1_BtagDown->GetBinContent(htBin,metBin)));
        }
  }
  if(fileName.find("WToLNu")!=string::npos ){
        ...
        if (Met_pt>700 && Met_pt<1600) metBin = 3;
        if (htBin==0 || metBin==0){
           w_SF2=1; w_SF2Up=1; w_SF2Down=1; w_SF2_BtagUp=1; w_SF2_BtagDown=1;
        }  else {
           ...
           //w_SF2_BtagUp =  histoSF2_BtagUp->GetBinContent(htBin,metBin);
           //w_SF2_BtagDown =  histoSF2_BtagDown->GetBinContent(htBin,metBin);
           w_SF2_BtagUp = histoSF2->GetBinContent(htBin,metBin) + fabs(histoSF2_BtagUp->GetBinContent(htBin,metBin)-histoSF2->GetBinContent(htBin,metBin));
           w_SF2_BtagDown = TMath::Max(0.,histoSF2->GetBinContent(htBin,metBin)-fabs(histoSF2->GetBinContent(htBin,metBin)-histoSF2_BtagDown->GetBinContent(htBin,metBin)));
        }
  }
  if(fileName.find("TT")!=string::npos||fileName.find("tZq")!=string::npos||fileName.find("ST")!=string::npos ){
        ...
        if (Met_pt>700 && Met_pt<1600) metBin = 3;
        if (htBin==0 || metBin==0){
           w_SF3=1; w_SF3Up=1; w_SF3Down=1; w_SF3_BtagUp=1;  w_SF3_BtagDown=1;
        }  else {
           ...
           //w_SF3_BtagUp =  histoSF3_BtagUp->GetBinContent(htBin,metBin);
           //w_SF3_BtagDown =  histoSF3_BtagDown->GetBinContent(htBin,metBin);
           w_SF3_BtagUp = histoSF3->GetBinContent(htBin,metBin) + fabs(histoSF3_BtagUp->GetBinContent(htBin,metBin)-histoSF3->GetBinContent(htBin,metBin));
           w_SF3_BtagDown = TMath::Max(0.,histoSF3->GetBinContent(htBin,metBin) - fabs(histoSF3->GetBinContent(htBin,metBin)-histoSF3_BtagDown->GetBinContent(htBin,metBin)));
        }
  }
...
void branch(bool data, TTree *NewTree,TTree *NewTreeSB, string fileName){
  ...
  NewTree->Branch("w_ZToNuNu_BtagUp",       &w_ZToNuNu_BtagUp,       "w_ZToNuNu_BtagUp/F"       );
  NewTree->Branch("w_ZToNuNu_BtagDown",     &w_ZToNuNu_BtagDown,     "w_ZToNuNu_BtagDown/F"     );
  ...
  NewTree->Branch("w_WToLNu_BtagUp",        &w_WToLNu_BtagUp,        "w_WToLNu_BtagUp/F"        );
  NewTree->Branch("w_WToLNu_BtagDown",      &w_WToLNu_BtagDown,      "w_WToLNu_BtagDown/F"      );
  ...
  NewTree->Branch("w_ttbar_BtagUp",         &w_ttbar_BtagUp,         "w_ttbar_BtagUp/F"         );
  NewTree->Branch("w_ttbar_BtagDown",       &w_ttbar_BtagDown,       "w_ttbar_BtagDown/F"       );
...
void newPUWeight(float &puweight,float &puweightUP,float &puweightDOWN){
  ...
  unsigned int nPUMax = 100;
  double npu_mix_2018_25ns_JuneProjectionFull18_PoissonOOTPU[100] = {
  ...
  npuProbs = npu_mix_2018_25ns_JuneProjectionFull18_PoissonOOTPU;
